...
describe('Express - Deviation Guards', function () {
  ...
  async function bootstrapWithPendingRedeems(redeemBps: number) {
    ...
    await express
      .connect(user1)
      .requestDeposit(await usdo.getAddress(), DEPOSIT_AMT, user1.address);
    await express.connect(maintainer).processDepositQueue(1, DEPOSIT_AMT);
    // Deposit user2
    await express
      .connect(user2)
      .requestDeposit(await usdo.getAddress(), DEPOSIT_AMT, user2.address);
    await express.connect(maintainer).processDepositQueue(1, DEPOSIT_AMT);
  ...
  describe('Setters', function () {
    ...
    it('reverts when caller lacks MAINTAINER_ROLE', async function () {
      ...
      await expect(
        express.connect(user1).updateDepositMaxDeviationBps(100)
      ).to.be.revertedWithCustomError(express, 'AccessControlUnauthorizedAccount');
      await expect(
        express.connect(user1).updateRedeemMaxDeviationBps(100)
      ).to.be.revertedWithCustomError(express, 'AccessControlUnauthorizedAccount');
    });
  ...
  describe('processPendingRedeems - deviation', function () {
    ...
    it('within +1% band succeeds', async function () {
      ...
      await expect(express.connect(operator).processPendingRedeems(2, totalAsset)).to.not.be
        .reverted;
    });
    ...
    it('outside band (over) reverts OracleDeviationExceeded', async function () {
      ...
      await expect(
        express.connect(operator).processPendingRedeems(2, totalAsset)
    ...
    it('bps == 0: 1 wei drift reverts', async function () {
      ...
      const expected = REDEEM_AMT_USER1 + REDEEM_AMT_USER2;
      await expect(
        express.connect(operator).processPendingRedeems(2, expected + 1n)
      ).to.be.revertedWithCustomError(express, 'OracleDeviationExceeded');
    });
    ...
    it('delay-not-elapsed at index 0 reverts NoPendingRedeemsReady', async function () {
      ...
      // Do NOT advance time
      await expect(
        express.connect(operator).processPendingRedeems(1, REDEEM_AMT_USER1)
  ...
  describe('processDepositQueue - deviation', function () {
    ...
    it('bps == 0: 1 wei drift reverts', async function () {
      ...
      await express
        ...
        .requestDeposit(await usdo.getAddress(), DEPOSIT_AMT, user1.address);
      await expect(
        express.connect(maintainer).processDepositQueue(1, DEPOSIT_AMT + 1n)
      ).to.be.revertedWithCustomError(express, 'OracleDeviationExceeded');
    });
    ...
    it('bps == 0: exact match succeeds', async function () {
      ...
      await express
        .connect(user1)
        .requestDeposit(await usdo.getAddress(), DEPOSIT_AMT, user1.address);
      await expect(express.connect(maintainer).processDepositQueue(1, DEPOSIT_AMT)).to.not.be
        .reverted;
    });
    ...
    it('within +1% band succeeds (was previously rejected as too-low _newShares)', async function () {
      ...
      await expect(express.connect(maintainer).processDepositQueue(1, newShares)).to.not.be
        .reverted;
    });
    ...
    it('outside band (over) reverts OracleDeviationExceeded', async function () {
      ...
      await expect(
        express.connect(maintainer).processDepositQueue(1, newShares)
      ).to.be.revertedWithCustomError(express, 'OracleDeviationExceeded');
    });
